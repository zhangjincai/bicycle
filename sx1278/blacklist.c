...
static int __sqlite3_create(struct bl *bl)
{
	assert(bl);
	...
	*/
	memset(sql_cmd, 0, sizeof(sql_cmd));
	sprintf(sql_cmd, "CREATE TABLE IF NOT EXISTS %s(ufile_idx INTEGER PRIMARY KEY DEFAULT 0, ufile BLOB)", TBL_TOTAL);
	...
	*/
	memset(sql_cmd, 0, sizeof(sql_cmd));
	sprintf(sql_cmd, "CREATE TABLE IF NOT EXISTS %s(ufile_idx INTEGER PRIMARY KEY DEFAULT 0, ufile BLOB)", TBL_INCREMENT);

	rc = sqlite3_exec(bl->db, sql_cmd, 0, 0, &errmsg);
	if(rc != SQLITE_OK)
	{
		fprintf(stderr, "Can't create table {%s}, errmsg = %s\n", TBL_INCREMENT, errmsg);
		if(errmsg != NULL)
		{
			sqlite3_free(errmsg);
			errmsg = NULL;
	...
	*/
	memset(sql_cmd, 0, sizeof(sql_cmd));
	sprintf(sql_cmd, "CREATE TABLE IF NOT EXISTS %s(ufile_idx INTEGER PRIMARY KEY DEFAULT 0, ufile BLOB)", TBL_DECREMENT);	

	rc = sqlite3_exec(bl->db, sql_cmd, 0, 0, &errmsg);
	if(rc != SQLITE_OK)
	{
		fprintf(stderr, "Can't create table {%s}, errmsg = %s\n", TBL_DECREMENT, errmsg);
		if(errmsg != NULL)
		{
			sqlite3_free(errmsg);
			errmsg = NULL;
	...
	*/	
	memset(sql_cmd, 0, sizeof(sql_cmd));
	sprintf(sql_cmd, "CREATE TABLE IF NOT EXISTS %s(ufile_idx INTEGER PRIMARY KEY DEFAULT 0, ufile BLOB)", TBL_TEMPORARY);	

	rc = sqlite3_exec(bl->db, sql_cmd, 0, 0, &errmsg);
	if(rc != SQLITE_OK)
	{
		fprintf(stderr, "Can't create table {%s}, errmsg = %s\n", TBL_TEMPORARY, errmsg);
		if(errmsg != NULL)
		{
			sqlite3_free(errmsg);
			errmsg = NULL;
...
lib_bl_t *lib_bl_create(const char *pathname)
{
	assert(pathname);	
...
void lib_bl_destroy(lib_bl_t *bl)
{
	assert(bl);
...
int lib_bl_insert_data(lib_bl_t *bl, const unsigned char bl_type, const unsigned int ufile_idx, univ_file_st_t *univ)
{
	if((bl == NULL) || (univ == NULL))
		return LIB_BL_ERROR;

	if(bl_type >= BL_TYPE_END)
		return LIB_BL_ERROR;
	...
	int file_len = sizeof(univ_file_st_t);
	...
	switch(bl_type)
	{
		...
		case BL_TYPE_INCREMENT:
			strcpy(tbl, TBL_INCREMENT);
			break;
			
		case BL_TYPE_DECREMENT:
			strcpy(tbl, TBL_DECREMENT);
			break;
			
		case BL_TYPE_TEMPORARY:
			strcpy(tbl, TBL_TEMPORARY);
			break;
	...
	sprintf(sql_cmd, "INSERT INTO %s(ufile_idx, ufile) VALUES (?,?);", tbl);
	...
	sqlite3_bind_int(stmt, 1, ufile_idx);
	sqlite3_bind_blob(stmt, 2, univ, file_len, SQLITE_STATIC);
...
int lib_bl_update_data(lib_bl_t *bl, const unsigned char bl_type, const unsigned int ufile_idx, univ_file_st_t *univ)
{
	if((bl == NULL) || (univ == NULL))
		return LIB_BL_ERROR;
	...
	sprintf(sql_cmd, "UPDATE %s SET ufile=? WHERE ufile_idx=%u;", tbl, ufile_idx);
	...
	sqlite3_bind_blob(stmt, 1, univ, file_len, SQLITE_STATIC);
...
int lib_bl_replace_data(lib_bl_t *bl, const unsigned char bl_type, const unsigned int ufile_idx, univ_file_st_t *univ)
{
	if((bl == NULL) || (univ == NULL))
		return LIB_BL_ERROR;
	...
	sprintf(sql_cmd, "REPLACE INTO %s(ufile_idx, ufile) VALUES (?,?);",  tbl);
...
int lib_bl_select_data(lib_bl_t *bl, const unsigned char bl_type, const unsigned int ufile_idx, univ_file_st_t *univ)
{
	if((bl == NULL) || (univ == NULL))
		return LIB_BL_ERROR;
	...
	sprintf(sql_cmd, "SELECT * FROM %s WHERE ufile_idx = %u;", tbl, ufile_idx);
	...
	if(rc == SQLITE_ROW)  //如果rc==SQLITE_DONE，说明查询失败
	{
		...
		puniv = (univ_file_st_t *)sqlite3_column_blob(stmt, 1);

		if(puniv != NULL)
		{
			memcpy(univ, puniv, sizeof(univ_file_st_t));
	...
	return LIB_BL_DONE;	//查询失败
}

int lib_bl_delete_table(lib_bl_t *bl, const unsigned char bl_type)
{
	if(bl == NULL)
		return LIB_BL_ERROR;
...
int lib_bl_delete_table_any(lib_bl_t *bl, const unsigned char bl_type, const unsigned int ufile_idx)
{
	if(bl == NULL)
		return LIB_BL_ERROR;
	...
	sprintf(sql_cmd, "DELETE FROM %s WHERE ufile_idx = %u;", tbl, ufile_idx);
...
int lib_bl_count(lib_bl_t *bl, const unsigned char bl_type, unsigned int *count)
{
...
int lib_bl_max(lib_bl_t *bl, const unsigned char bl_type, unsigned int *max)
{
	if((bl == NULL) || (max == NULL))
		return LIB_BL_ERROR;
	...
	sprintf(sql_cmd, "SELECT MAX(ufile_idx) FROM %s;", tbl);
...
static int __sqlite3_create_v2(struct bl *bl)
{
	...
	*/
	...
	sprintf(sql_cmd, "CREATE TABLE IF NOT EXISTS %s(idx INTEGER PRIMARY KEY AUTOINCREMENT, \
					pid CHAR(8), lid CHAR(8), type TEXT)", TBL_TOTAL);
	#else
	sprintf(sql_cmd, "CREATE TABLE IF NOT EXISTS %s(idx INTEGER PRIMARY KEY AUTOINCREMENT, \
