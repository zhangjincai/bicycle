...
int lib_gui_unity_file_info_get(lib_gui_t *gui, gui_unity_file_info_t *file, const unsigned int msec)
{
	...
	if(ret > 0)
	{
		...
		//lib_printf_v2("unity_file_info", file, sizeof(file.stake_ufile_st[5])) //for test 
...
#if 0
/* 读卡器升级结果通知 add by zjc at 2016-07-25 */
int lib_gui_lnt_update_result_notify(lib_gui_t *gui, struct lnt_firmware_update_result *result, const unsigned int msec)
{
	if((gui == NULL) || (result == NULL))
		return LIB_GUI_ERROR;
	
	int ret = LIB_GUI_ERROR;
	int length = sizeof(struct lnt_firmware_update_result);
	gui_package_t pkg;
	int pkg_len = sizeof(struct gui_package);
	memset(&pkg, 0, pkg_len);
	
	__gui_package_create_with_data(&pkg, GUI_CMD_LNT_UPDATE_RESULT_NOTIFY, result, length, GUI_RESULT_OK);
	...
	if(ret > 0)
	{
		...
		if(__is_gui_package(&pkg, GUI_CMD_LNT_UPDATE_RESULT_NOTIFY) != LIB_GUI_TRUE)
			return LIB_GUI_ERROR;
